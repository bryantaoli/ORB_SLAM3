...
public:
    ...
    /**
     * @brief 将Eigen矩阵转换成为cv::Mat格式
     * 
     * @param[in] 类型为Eigen::MatrixXd的矩阵
     * @return cv::Mat 转换结果
     */
    ...
    /**
     * @brief 将给定的向量转换为其反对称矩阵
     * @details 输入的向量必须是3维向量
     * @param[in] cv::Mat 3*1的向量
     * @return cv::Mat 3*3的反对称矩阵
     */
     ...
     * 
     * @param[in] cvMat4 输入
     * @return Eigen::Matrix<double,4,4> 转换结果
     */
    ...
    /**
     * @brief 判断给定的矩阵是否为SO(3)旋转矩阵
     * 
     * @param[in] R 以cv::Mat表示的矩阵
     * @return bool 真或者假
     */
    static bool isRotationMatrix(const cv::Mat &R);

    /**
     * @brief 将给定的cv::Mat类型的旋转矩阵转换成std::vector<float>表示的欧拉角
     * 
     * @param[in] R 以cv::Mat表示的旋转矩阵
