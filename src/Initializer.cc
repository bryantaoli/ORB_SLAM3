                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    if(RH>0.40) // if(RH>0.40)
    {
        ...
        return ReconstructH(vbMatchesInliersH,  //输入，匹配成功的特征点对Inliers标记
                            ...
                            minParallax, //minParallax可以取1.0，即认为某对特征点的三角化测量中，认为其测量有效时；需要满足的最小视差角（如果视差角过小则会引起非常大的观测误差）,单位是角度
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // Step 4 利用重投影误差为当次RANSAC的结果评分
        currentScore = CheckHomography(H21i, //输入，单应矩阵的计算结果
                                       H12i, //输入，单应矩阵的计算结果
                       ...
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        //? bug 我觉得这个写的位置不太对。good点计数都++了然后才判断，不是会让good点标志和good点计数不一样吗
