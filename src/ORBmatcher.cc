    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        if(pMP->mbTrackInView)
        {
            ...
            const vector<size_t> vIndices =
                    F.GetFeaturesInArea(pMP->mTrackProjX,   // 该地图点投影到一帧上的坐标
                                        pMP->mTrackProjY,   // 该地图点投影到一帧上的坐标
                                        r*F.mvScaleFactors[nPredictedLevel],        // 认为搜索窗口的大小和该特征点被追踪到时所处的尺度也有关系
                                        nPredictedLevel-1,  // 搜索的图层范围
                                        nPredictedLevel);   // 搜索的图层范围
                ...
                if(bestDist1<=TH_LOW)
                {
                    ...
                    if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                    {
                        ...
                        if(mbCheckOrientation)
                        {
                            ...
                            float rot = kp.angle-Fkp.angle; // 该特征点的角度变化值
                            if(rot<0.0)
                                rot+=360.0f;
...
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
{
    ...
    cv::Mat tcw = Scw.rowRange(0,3).col(3)/scw; // 除掉
    cv::Mat Ow = -Rcw.t()*tcw;
